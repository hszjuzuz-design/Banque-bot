...
const { getCompte, setLastWork, enregistrerGainTravail } = require('../database');
const { getMetier } = require('../metiers');
const { verifierDeblocages } = require('../utils/progression');
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const gain = Math.max(1, Math.round(metier.gainBase * facteur));

    setLastWork(userId, guildId, maintenant);
    enregistrerGainTravail(userId, guildId, gain);
